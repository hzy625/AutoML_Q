...
filepath = 'MF_LargeCap_ExcessReturn_3Y.parquet'


def prepare_data_for_er_ari(filepath, label_window):
    ...
    for ticker, series in tqdm(data_dict.items()):
        if (series.isna().sum() == series.shape[0])|(series.shape[0] == 0):
            tickers_to_remove += [ticker]
            continue

        last_date = series.index[-1] - relativedelta(years=label_window)
        if last_date <= series.index[0]:
            tickers_to_remove.append(ticker)
            continue

        index = series.loc[:series.index[-1] - relativedelta(years=label_window)].index
        label_dict[ticker] = pd.Series([
            series[date + relativedelta(years=label_window)] for date in index
        ], index=index)
...
def get_data(filepath):
    ...
    data_dict, label_dict = prepare_data_for_er_ari(filepath, pm_window)
    ...
    Xte = np.concatenate(test_data)
    Yte = np.concatenate(test_labels)

    Xtr, Xval, Ytr, Yval = train_test_split(train_data, train_labels, train_size=0.9)
