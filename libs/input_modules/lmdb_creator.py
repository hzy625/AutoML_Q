...
def create_record(X, y, path
    ...
    with env.begin(write=True) as txn:
        ...
        # cite: https://stackoverflow.com/questions/31774953/test-labels-for-regression-caffe-float-not-allowed/31808324#31808324
        # cite: https://stackoverflow.com/questions/32707393/whats-caffes-input-format
        
        for idx, x in enumerate(X): 
            content_field = np.array([x]+y[idx])
            #print('original shape:',content_field.shape)
            # get shape (1,1,1)
            content_field = np.expand_dims(content_field, axis=1)
            #print('shape after expansion:',content_field.shape)
            ...
            txn.put('{:0>7d}'.format(idx), dat.SerializeToString())
...
def main():
    ...
    parser.add_argument('-v','--number_val', help='How many validation data points', type=int, default=0)
    '''
    ...
    if args.dataset == 'AutoML':
        Xtr, Ytr, Xval, Yval, Xte, Yte = get_datasets.get_data(filepath=args.original_data_dir)
