...
class LstmScanner(runtime.Scanner):
    patterns = [
        ('"\\\\]"', re.compile('\\]')),
        ('"\\\\["', re.compile('\\[')),
        ('"\\\\)"', re.compile('\\)')),
...
class Lstm(runtime.Parser):
    ...
    def numlist(self, _parent=None):
        ...
        self._scan('"\\\\("', context=_context)
        result = []
        # if peeked NUM -> append scanned numerical value to result
        NUM = self._scan('NUM', context=_context)
        result.append(float(NUM))
        # else if peeked ACTIV -> append activation function string to result
        while self._peek('"\\\\)"', '","', context=_context) == '","':
            self._scan('","', context=_context)
...
def parse(rule, text):
    P = Lstm(LstmScanner(text))
    ## to check: how to replace the line above for CnnScanner
