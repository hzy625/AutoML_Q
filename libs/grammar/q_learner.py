...
# import cnn
import lstm # to check: to be created
...
class QLearner:
    ...
    def __init__(self,
        ...
        self.state = se.State('start', 0, 0, None, 0, 0, 0, 0) if not state else state
        # to check: is the line above correct?
        # should we always have a state parameter input as starting state to construct the QLearner instance

        #self.bucketed_state = self.enum.bucket_state(self.state)
        #I commented the line above because it is irrelevant for our case.
    ...
    def _reset_for_new_walk(self):
        ...
        self.state = se.State('start', 0, 0, None, 0, 0, 0, 0) #to check
        # self.bucketed_state = self.enum.bucket_state(self.state) # not used here
    ...
    def _transition_q_learning(self):
        ...
        # to check: I modified the if statement above as we are not using bucketed_state
    ...
    def sample_replay_for_update(self):
        ...
        for i in range(self.state_space_parameters.replay_number):
            ...
            state_list = self.stringutils.convert_model_string_to_states(lstm.parse('net', net))

            ## to check: we don't do remove_drop_out_states because we are not adding dropout layers mechanically by rule.
